...
from dataHandler.loadData import Batch
...
class Model:
    def __init__(self,
        ...
        self.sess, self.saver = self.setTF()

    def setCNN(self) -> None:
    ...
    def setTF(self) -> Tuple[tf.compat.v1.Session, tf.compat.v1.train.Saver]:
    ...
    def sparse(self, texts: List[str]) -> Tuple[List[List[int]], List[int], List[int]]:
        indices = []
        values = []
        shape = [len(texts), 0]  
    ...
    def decodeToText(self, ctc_output: tuple, batch_size: int) -> List[str]:
    ...
    def trainBatch(self, batch: Batch) -> float:

        num_batch_elements = len(batch.imgs)
        max_text_len = batch.imgs[0].shape[0] // 4
        sparse = self.sparse(batch.gt_texts)
    ...
    def inferBatch(self, batch: Batch, calc_probability: bool = False, probability_of_gt: bool = False):
        num_batch_elements = len(batch.imgs)
        eval_list = []
        ...
        texts = self.decodeToText(decoded, num_batch_elements)

        probs = None
        if calc_probability:
            sparse = self.sparse(batch.gt_texts) if probability_of_gt else self.sparse(texts)
